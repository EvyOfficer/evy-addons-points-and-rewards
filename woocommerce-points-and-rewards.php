<?php
/**
 * Plugin Name: Evy - Points and Rewards
 ...
 * Version: 1.0.0 (Based 1.8.17)
 * Text Domain: evy-addons-points-and-rewards
...
/**
 * Schedule the daily points expiry event.
 * Erase any existing event(s) and reschedule the next one.
 */
function wc_points_rewards_expire_points_schedule() {
	$hook = 'wc_points_rewards_expire_points_daily';
...
if ( ! class_exists( 'WC_Points_Rewards' ) ) :
	define( 'WC_POINTS_REWARDS_VERSION', '1.8.17' ); // WRCS: DEFINED_VERSION.
	...
	class WC_Points_Rewards {
		...
		public static function setup_blocks_integration() {
			if ( ! class_exists( 'Automattic\WooCommerce\Blocks\Package' ) || ! version_compare( \Automattic\WooCommerce\Blocks\Package::get_version(), '9.6.0', '>' ) ) {
				return;
			}
		...
		public function includes() {
			...
			require_once __DIR__ . '/includes/class-wc-points-rewards-product-block-editor.php';
			new WC_Points_Rewards_Product_Block_Editor();
		...
		/**
		 * Initializes the extension once activated plugins have loaded.
		 *
		 * @since 1.6.32
		 * @return Object Instance of extension.
		 */
		public static function woocommerce_points_and_rewards_plugins_loaded() {
		...
		/**
		 * Initializes the extension after WordPress has finished loading.
		 *
		 * @since 1.8.11
		 */
		public static function woocommerce_points_and_rewards_init() {
			load_plugin_textdomain( 'woocommerce-points-and-rewards', false, plugin_basename( __DIR__ ) . '/languages' );
		}

		public static function init() {
			add_action( 'plugins_loaded', array( __CLASS__, 'woocommerce_points_and_rewards_plugins_loaded' ) );
			add_action( 'init', array( __CLASS__, 'woocommerce_points_and_rewards_init' ) );
		}
	}
endif;
